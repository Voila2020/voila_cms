...
class WebsiteLanguagesController extends CBController
{
    ...
    {
        if (Request::input('feild') == 'default' && Request::input('value') == 1) {
            $defaultLangs = DB::table(Request::input('table'))
                ->where('default', 1)
                ->get();
            if ($defaultLangs->count() > 0) {
                throw new Exception('Default language already exists!');
            } else {
                parent::postEditSwitchAction($request);
            }

...
class AdminFooterMenusController extends CBController
{
    ...
    public function getIndex()
    {
        ...
        $menu_active = Menu::where('type', 'footer')
            ->whereNull('parent_id')
            ->where('active', 1)
            ->orderBy('sorting', 'asc')
            ->with('children') // Eager load children menus
            ->get();
        $menu_inactive = Menu::withoutGlobalScope('active')->where('type', 'footer')
            ->whereNull('parent_id')
            ->where('active', 0)
            ->orderBy('sorting', 'asc')
            ->get();
        foreach ($menu_inactive as &$menu) {
            $child = Menu::where('type', 'footer')
                ->where('active', 1)
        ...
        return $this->view('crudbooster::footermenus.index', ['menu_active' => $menu_active, 'menu_inactive' => $menu_inactive, 'return_url' => $return_url, 'page_title' => $page_title, 'current_lang' => $current_lang]);
    }
    //-----------------------------------------------//
    private function getMenuChildren($menu)
    {
        $children = Menu::where('type', 'footer')
            ->where('active', 1)
            ->where('parent_id', $menu->id)
            ->orderBy('sorting', 'asc')
            ->get();
    ...
    {
        DB::table('menus')->where('type', 'footer')->where('parent_id', $id)->delete();
    }
    //-----------------------------------------------//

<?php
	...
	******/
	...
	class Compressor{
		...
		public $start_time;
        public $file_loaded_size = 0;
        public $script_list = "";
        public $path;
        public $load_all_plugins;
        public $full_cache_file;
        public $gzip_cache_file;
        public $use_gzip;
        public $gzip_enc_header;
        public $cache_file;
        public $datas;
        public $sub_script_list;
        public $infos;
        public $gzip_datas;
        function __construct(public $param)
		{
		...
		function check_gzip_use()
		{
			...
			if (isset($_SERVER['HTTP_ACCEPT_ENCODING'])) {
                $encodings = explode(',', strtolower((string) preg_replace("/\s+/", "", (string) $_SERVER['HTTP_ACCEPT_ENCODING'])));
            }
			
			// desactivate gzip for IE version < 7
			if(preg_match("/(?:msie )([0-9.]+)/i", $_SERVER['HTTP_USER_AGENT'], $ie) && $ie[1] < 7)
			{
			...
			if (file_exists($this->cache_file)) {
				...
				if($this->load_all_plugins){
					...
					if (($dir = @opendir($plug_path)) !== false)
					{
						while (($file = readdir($dir)) !== false)
						{
							if ($file !== "." && $file !== ".." && (is_dir($plug_path.$file) && file_exists($plug_path.$file."/".$file.".js")))
							{
				...
				if(filemtime($this->cache_file) >= $cache_date){
					...
					if (isset($_SERVER["HTTP_IF_MODIFIED_SINCE"]) && strcasecmp((string) $_SERVER["HTTP_IF_MODIFIED_SINCE"], $last_modified) === 0)
					{
		...
		function load_files()
		{
	    	...
	    	$loader= preg_replace_callback("/(t\\.scripts_to_load=\\s*)\\[([^\\]]*)\\];/"
						, fn($matches) => $this->replace_scripts('script_list', $matches[1], $matches[2])
						, $loader);
		
			$loader= preg_replace_callback("/(t\\.sub_scripts_to_load=\\s*)\\[([^\\]]*)\\];/"
						, fn($matches) => $this->replace_scripts('sub_script_list', $matches[1], $matches[2])
						, (string) $loader);
			...
			if (($dir = @opendir($reg_path)) !== false)
			{
				while (($file = readdir($dir)) !== false)
				{
					if( $file !== "." && $file !== ".." && ( $pos = strpos( $file, '.js' ) ) !== false )
					{
						...
						if( preg_match( '@(\'|")DISPLAY_NAME\1\s*:\s*(\'|")(.*)\2@', (string) $jsContent, $match ) )
						{
			...
			$this->datas= preg_replace(["/(\b)EditAreaLoader(\b)/", "/(\b)editAreaLoader(\b)/", "/(\b)editAreas(\b)/"], ["EAL", "eAL", "eAs"], (string) $this->datas);
...
//			die;
		...
		{
			$end_uri=preg_replace("/\.\./", "", (string) $end_uri); // Remove any .. (security)
		...
		function compress_javascript(&$code)
		{
			if($this->param['compress'])
			{
				...
				$code= preg_replace('/( |\t|\r)*(;|\{|\}|=|==|\-|\+|,|\(|\)|\|\||&\&|\:)( |\t|\r)*/', "$2", (string) $code);
			}
		}
		...
		function prepare_string_for_quotes(&$str){
			...
			$str= preg_replace($pattern, $replace, (string) $str);
		}
		
		function replace_scripts($var, $param1, $param2)
		{
			$this->$var=stripslashes((string) $param2);
	        return $param1."[];";
		}

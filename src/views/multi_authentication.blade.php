...
<head>
    <meta charset="UTF-8">
    <title>{{ cbLang('page_title_multi_authentication') }} : {{ $appname ?? config('app.name', 'CRUDBooster') }}</title>
...
<body class="login-page">
    <div class="login-box">
        ...
        <div class="login-box-body">
            ...
            <p class="login-box-msg">{{ cbLang('multi_authentication_message') }}</p>
            <form action="{{ route('postMultiAuthentication') }}" method="post">
                <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                <input type="hidden" name="email" value="{{ $email??Session::get('email') }}" />
                <div class="form-group has-feedback">
                    <input type="text" class="form-control" name='auth_code' required placeholder="Authentication Code" />
                    <span class="glyphicon glyphicon-edit form-control-feedback"></span>
                </div>
                <div class="row">
                    <div class="col-xs-8">
                        {{ cbLang('multi_authentication_text_try_again') }} <a

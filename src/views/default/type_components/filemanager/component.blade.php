...
@else
    ...
    @endphp
    <div class='form-group filemanager-form-group_{{ $name . '_' . $current_language->code }} {{ $header_group_class }} {{ $errors->first($name . '_' . $current_language->code) ? 'has-error' : '' }}'
        id='form-group-{{ $name . '_' . $current_language->code }}' style='{{ @$form['style'] }}'>
        <label class='control-label col-sm-2'>{{ $form['label'] . ' - ' . $current_language->name }}
            @if ($required)
                <span class='text-danger' title='{!! cbLang('this_field_is_required') !!}'>*</span>
            @endif
        </label>

        <div class="{{ $col_width ? $col_width . ' empty-filemanager-col_' . $name . '_' . $current_language->code : 'col-sm-10 filemanager-col_' . $name . '_' . $current_language->code }}"
            style="{{ $value ? 'display: none' : '' }}">
            <div class="input-group">
                <input id="{{ $name . '_' . $current_language->code }}"
                    filemanager_type="{{ @$form['filemanager_type'] == 'file' ? 'file' : 'image' }}"
                    class="form-control hide" type="text" value='{{ $value }}' name="{{ $name . '_' . $current_language->code }}">

                <a data-lightbox="roadtrip" class="hide" id="link-{{ $name . '_' . $current_language->code }}" href=""
                    style="{{ @$form['filemanager_type'] == 'file' ? 'pointer-events: none;' : '' }}">
                    <img style="width:150px;height:150px; {{ @$form['filemanager_type'] == 'file' ? 'display:none;' : '' }}"
                        id="img-{{ $name . '_' . $current_language->code }}" title="Add image for {{ $name . '_' . $current_language->code }}" src="">
                    <p class="file-roadtrip" id="file-{{ $name . '_' . $current_language->code }}"
                        style="{{ @$form['filemanager_type'] == 'file' ? '' : 'display:none;' }}"></p>
                </a>

                <span class="input-group-btn">
                    @if (@$form['filemanager_type'] == 'file')
                        <a id="_{{ $name . '_' . $current_language->code }}" onclick='OpenInsertImagesingle_{{$current_language->code}}("{{ $name . "_" . $current_language->code }}")'
                            class="btn btn-primary" value="file_type">
                            <i class="fa fa-file-o"></i> {{ cbLang('chose_an_file') }}
                        @else
                            <a id="_{{ $name . '_' . $current_language->code }}" onclick='OpenInsertImagesingle_{{$current_language->code}}("{{ $name . "_" . $current_language->code }}")'
        ...
        <div class="{{ $col_width ? $col_width . ' filemanager-col_' . $name . '_' . $current_language->code : 'col-sm-10 filemanager-col_' . $name . '_' . $current_language->code }}"
            style="{{ $value ? '' : 'display: none;' }}">
            <input id="thumbnail-{{ $name . '_' . $current_language->code }}" class="form-control" type="hidden" value='{{ $value }}'
                name="{{ $name . '_' . $current_language->code }}">
            @if (@$form['filemanager_type'] == 'file')
                @if ($value)
                    <div style='margin-top:15px'><a id='holder-{{ $name . '_' . $current_language->code }}' href='{{ asset($value) }}'
                            target='_blank' title=' {{ cbLang('button_download_file') }} {{ basename($value) }}'><i
                                class='fa fa-download'></i> {{ cbLang('button_download_file') }}
                            {{ basename($value) }}</a>
                    </div>
                @endif
            @else
                <p><a id="roadtrip-{{ $name . '_' . $current_language->code }}" class="p-roadtrip" data-lightbox="roadtrip"
                        href="{{ $value ? asset($value) : '' }}"><img id='holder-{{ $name . '_' . $current_language->code }}'
                            src="{{ $value ? asset($value) : '' }}" style="margin-top:15px;max-height:100px;"></a>
                </p>
            @endif
            @if (!$readonly || !$disabled)
                <p>
                    <a class='btn btn-danger btn-delete btn-sm btn-del-filemanager'
                        onclick='showDeletePopout_{{$current_language->code}}("{{ $name . "_" . $current_language->code }}")'><i class='fa fa-ban'></i>
                        {{ cbLang('text_delete') }} </a>
                </p>
            @endif
            <div class='help-block'>{{ @$form['help'] }}</div>
    ...
    <div class="modal fade" id="modalInsertPhotosingle_{{ $name . '_' . $current_language->code }}">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="buttons">
    ...
    @push('bottom')
        <script type="text/javascript">
            ...
            function OpenInsertImagesingle_{{$current_language->code}}(name) {
                Name = name;
                // reback size of iframe to default
                $('.modal.in .modal-dialog').width(900);
                // check file manager type
                ...
                $('#roadtrip-' + name).prop("href", "");
                $('#holder-' + name).prop("src", "");
                $("#modalInsertPhotosingle_{{ $name . '_' . $current_language->code }} .modal-body").html(link);
                $("#modalInsertPhotosingle_{{ $name . '_' . $current_language->code }}").modal();
            }

            function showDeletePopout_{{$current_language->code}}(name) {
                swal({
                    title: "{{ cbLang('delete_title_confirm') }}",
                    ...
                    deleteImage_{{$current_language->code}}(name);
                });
            }


            function deleteImage_{{$current_language->code}}(form_name) {
                let currUrl = @json(CRUDBooster::mainpath()) + '/update-single';
                let table = @json($table);
                let id = @json($id);
                let lang = @json($current_language->code);
            ...
            $(function() {
                var id = '#modalInsertPhotosingle_{{ $name . '_' . $current_language->code }}';
                $(id).on('hidden.bs.modal', function() {
                    var check = $('#' + Name).val();
                    if (check != "") {
                        check = check.substring(1);

                                            ...
                                            @if ($col['type'] == 'filemanager')
                                                <div class="{{ 'col-sm-10 filemanager-col_' . $col['name'] }}"
                                                    ...
                                                    <div class="input-group">
                                                        <input id="{{ $col['name'] }}" class="form-control hide"
                                                        ...
                                                        $(function() {
                                                            $(id).on('hidden.bs.modal', function() {
                                                                ...
                                                                if (check != "") {
                                                                    ...
                                                                    if ("{{ $col['filemanager_type'] }}" == 'file')
                                                                        $("#file-{{ $col['name'] }}").html(check);
                                                                    else
                                                                        $("#img-{{ $col['name'] }}").attr("src", '{{ URL::asset('') }}' + check);
                                                                    $("#link-{{ $col['name'] }}").attr("href", check);
                                                                    $("#link-{{ $col['name'] }}").removeClass("hide");
                                                                    $("#thumbnail-{{ $col['name'] }}").attr("src", '{{ URL::asset('') }}' + check);
                                                                    $("#thumbnail-{{ $col['name'] }}").attr("value", check);
                                                                }
                                                            });
                                                ...
                                                @foreach ($form['columns'] as $c)
                                                    @if ($c['type'] == 'filemanager')
                                                        trRow += "<td class='{{ $c['name'] }}'>" +
                                                            "<a data-lightbox='roadtrip' href='" + $(
                                                                '#img-{{ $c['name'] }}').attr('src') + "'><img class='tb_img-" + '{{ $c['name'] }}' +
                                                            "' data-label='" + $(
                                                                '#{{ $name . $c['name'] }} .input-label').val() + "' src='" + $(
                                                                '#img-{{ $c['name'] }}').attr('src') + "' width='50px' height='50px'/></a>" +
                                                    ...
                                                    @elseif ($c['type'] == 'switch')
                                                        trRow += "<td class='{{ $c['name'] }}' value='" +
                                                            ...
                                                            "</td>";
                                                    @elseif ($c['type'] == 'icon')
                                                        // console.log($('select[name=child_icon_{{ $c['name'] }}]').val())
                                    ...
                                    <tr>
                                        ...
                                        @foreach ($form['columns'] as $col)
                                            <td class="{{ $col['name'] }}">
                                                <?php
                                                if ($col['type'] == 'filemanager') {
                                                    echo "<a data-lightbox='roadtrip' href='" . $d->{$col['name']} . "'>";
                                                    echo "<img data-label='undefined' class='tb_img-" . $col['name'] . "' src='" . $d->{$col['name']} . "' width='50px' height='50px'>";
                                                    echo '</a>';

...
@else
    ...
    <div class='form-group {{ $header_group_class }} {{ $errors->first($name . '_' . $current_language->code) ? 'has-error' : '' }}'
        ...
        @endif
        <label class='control-label col-sm-2'>
            {{ cbLang($form['label']) . ' - ' . $current_language->name }}
            @if ($required)
                <span class='text-danger' title='{!! cbLang('this_field_is_required') !!}'>*</span>
            @endif
        </label>
        ...
        <div class="{{ $col_width ?: 'col-sm-10' }}">
            <input type='text' title="{{ $form['label'] }}" {{ $required }} {{ $readonly }}
                {!! $placeholder !!} {{ $disabled }}
                {{ $validation['max'] ? 'maxlength=' . $validation['max'] : '' }} class='form-control'
                name="{{ $name . '_' . $current_language->code }}" id="{{ $name . '_' . $current_language->code }}"
                value='{{ $value }}' />

            <div class="text-danger">{!! $errors->first($name . '_' . $current_language->code)

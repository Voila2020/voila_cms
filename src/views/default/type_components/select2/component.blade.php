...
@else
    @foreach ($websiteLanguages as $lang)
                                    ...
                                    initSelection: function(element, callback) {
                                        ...
                                        if (id !== '') {
                                            $.ajax('{{ $url }}', {
                                                ...
                                                callback(data.items[0]);
                                                $('#<?php echo $name. "_" . $lang->code; ?>').html("<option value='" + data.items[0]
                                                    .id + "' selected >" + data.items[0].text + "</option>");
                                            });
                                        }
                                    }
        ...
        <div class='form-group {{ $header_group_class }} {{ $errors->first($name. '_' . $lang->code) ? 'has-error' : '' }}'
            ...
            <div class="{{ $col_width ?: 'col-sm-10' }}">
                <select style='width:100%' class='form-control' id="{{ $name. '_' . $lang->code }}" {{ $required }} {{ $readonly }}
                    {!! $placeholder !!} {{ $disabled }}
                    name="{{ $name. '_' . $lang->code }}{{ $form['relationship_table'] || !empty($form['multiple']) ? '[]' : '' }}"
                    {{ $form['relationship_table'] || !empty($form['multiple']) ? 'multiple' : '' }}>
                ...
                <div class="text-danger">
                    {!! $errors->first($name. '_' . $lang->code) ? "<i class='fa fa-info-circle'></i> " . $errors->first($name. '_' . $lang->code) : '' !!}
                </div>
                <!--end-text-danger-->
                <p class='help-block'>{{ @$form['help'] }}</p>

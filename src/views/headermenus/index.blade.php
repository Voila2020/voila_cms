        ...
        if ($menu->children) {
            ...
            foreach ($menu->children as $child) {
                $privileges = DB::table('cms_menus_privileges')
                    ...
                    ->toArray();
                $editHref =
                    route('AdminHeaderMenusControllerGetEdit') . '/' . $child->id . '?return_url=' . urlencode(Request::fullUrl());
                $deleteClick = CRUDBooster::deleteConfirm(route('AdminHeaderMenusControllerGetDelete') . '/' . $child->id, true);
